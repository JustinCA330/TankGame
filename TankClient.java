...
public class TankClient extends Frame implements ActionListener {
	...
	public void framPaint(Graphics g) {
		...
		if(!Player2)g.drawString("Life: ", 580, 70);
		else g.drawString("Life: ", 95, 70);
		g.setFont(new Font("Arial", Font.PLAIN, 25));
		if(!Player2) g.drawString("" + homeTank.getLife(), 650, 70);
		else g.drawString("Player 1: " + homeTank.getLife()+"    Player 2:"+homeTank2.getLife(), 350, 70);
		g.setFont(f1);
		if (!Player2){
		...
		homeTank.draw(g);
		homeTank.eat(life);
		if (Player2) {homeTank2.draw(g);homeTank2.eat(life);}
